...
| **minizinc** | CP solver via MiniZinc (requires `pip install minizinc`, MiniZinc 2.6+ and Chuffed). |
| **combined** | Runs GRASP, SA, ACS, and ES in sequence; switches after stagnation (default 60 s). |

Default solver is **combined**. For **minizinc**, install the optional dependency: `pip install -e ".[minizinc]"` and install [MiniZinc](https://www.minizinc.org/) with the Chuffed backend. Use `--time-limit N`, `--max-generations N`, `--gen-min-improvement`, or `--time-min-improvement` to control stopping (see `python main.py --help`).